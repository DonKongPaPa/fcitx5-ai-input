...
#                   默认 $SRC/artifacts/dist/share/fcitx5-aiinput/flutter）
...
sed -i "s|ROOT=.*|ROOT=/usr$LIBDIR/fcitx5-aiinput|" \
    "$STAGE/usr$LIBDIR/funasr-server/funasr-serve.sh" 2>/dev/null || true

# 3. 文档
mkdir -p "$STAGE/usr/share/doc/fcitx5-ai-input"
cp "$SRC/LICENSE" "$STAGE/usr/share/doc/fcitx5-ai-input/" 2>/dev/null || true
